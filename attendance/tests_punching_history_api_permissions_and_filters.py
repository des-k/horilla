...
class AttendancePunchingHistoryApiIntegrationTests(AttendanceApiIntegrationMixin, APITestCase):
    ...
    def test_admin_api_sees_audit_fields(self):
        ...
        response = self._call(admin_user, params={'employee_id': owner.id, 'start_date': '2026-03-15', 'end_date': '2026-03-15'})
        data = self._json(response)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['results'][0]['id'], punch.id)
        self.assertIn('decision_source', data['results'][0])
        self.assertIn('work_mode', data['results'][0])

    def test_employee_api_hides_audit_fields(self):
        owner_user, owner = self.create_employee('Owner')
        AttendancePunchingHistory.objects.create(
            employee_id=owner,
            attendance_date=date(2026, 3, 15),
        ...
        response = self._call(owner_user, params={'start_date': '2026-03-15', 'end_date': '2026-03-15'})
        data = self._json(response)
        self.assertEqual(response.status_code, 200)
        self.assertNotIn('decision_source', data['results'][0])
        self.assertNotIn('work_mode', data['results'][0])

    def test_manager_api_hides_audit_fields(self):
        manager_user, manager = self.create_employee('Manager')
        _, subordinate = self.create_employee('Subordinate', manager=manager)
        AttendancePunchingHistory.objects.create(
            employee_id=subordinate,
        ...
        response = self._call(manager_user, params={'start_date': '2026-03-15', 'end_date': '2026-03-15'})
        data = self._json(response)
        self.assertEqual(response.status_code, 200)
        self.assertNotIn('decision_source', data['results'][0])
        self.assertNotIn('work_mode', data['results'][0])
