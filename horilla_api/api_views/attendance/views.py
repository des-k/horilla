...
from django.db import from datetime import date, datetime, timedelta, timezone
import json
...
class WorkModeRequestView(APIView):
    ...
    def post(self, request):
        ...
        # UX rule: reason/notes is mandatory.
        # (We still keep model field blank=True for backward compatibility.)
        reason = data.get("reason")
        if reason is None or str(reason).strip() == "":
            return Response(
                {"reason": ["Reason / Notes is required."]},
                status=status.HTTP_400_BAD_REQUEST,
            )
